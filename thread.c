// source: adapted from https://bytefreaks.net/programming-2/cc-set-affinity-to-threads-example-code
#include <stdio.h>
#include <stdlib.h>
#define __USE_GNU
#include <errno.h>
...
struct thread_info {
  ...
  unsigned int period_in_useconds;
};
...
void *thread_camper(void *arg)
{
  ...
  while(1) {
    hook(data, 6);
    usleep(thread_info->period_in_useconds);
  }
...
int main(int argc, char *argv[])
{
  ...
  unsigned int period_in_useconds = 1000000 / freq;
  printf("cpu: %d\tfreq: %d\tperiod: %d us\n", cpu, freq, period_in_useconds);
  ...
  for(tnum = 0; tnum < num_threads; tnum++) {
    ...
    thread_info[tnum].period_in_useconds = period_in_useconds;
